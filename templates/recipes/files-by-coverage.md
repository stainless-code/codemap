...
Aggregates the symbol-level `coverage` table by `file_path` (no separate `file_coverage` rollup table in v1; the `GROUP BY` is index-bounded by `idx_coverage_file_name`).
