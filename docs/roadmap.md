...
- [x] **Adaptive output budgets** — scale explore/trace/node snippet char caps and row limits from indexed file counts via **`resolveOutputBudget(file_count)`** in `output-budget.ts` (2026-05). **`context`** hub/signature caps remain in **`resolveContextBudget()`**.
