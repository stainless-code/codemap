...
function registerAuditTool(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        "Structural-drift audit. Composes per-delta snapshots (files / dependencies / deprecated) into a {head, deltas} envelope. Three snapshot sources, mutually exclusive at the top level: (1) `base: <ref>` — materialises a git committish (origin/main, HEAD~5, sha, tag) via `git worktree add` to a sha-keyed cache under `.codemap/audit-cache/`, reindexes into a temp DB, diffs against current. Cache hit on second run against same sha is sub-100ms. Requires a git repository — non-git projects get `{error: 'codemap audit: --base requires a git repository'}`. (2) `baseline_prefix` — auto-resolves <prefix>-{files,dependencies,deprecated} from `query_baselines`. (3) `baselines: {<deltaKey>: <name>}` — explicit per-delta overrides; composes orthogonally with both `base` and `baseline_prefix`. `summary: true` collapses each delta to {added: N, removed: N}. `no_index` controls the head-side incremental-index prelude (default re-indexes; watch-active default is no-op since the watcher keeps the index fresh; pass `no_index: false` to force).",
