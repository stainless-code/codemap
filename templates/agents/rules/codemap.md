...
| Recipe catalog (JSON)             | `codemap query --recipes-json` — every entry includes `last_run_at: number \| null` + `run_count: number` recency fields. Rank with `jq 'sort_by(.last_run_at // 0) \| reverse'`. Local-only; opt-out via `.codemap/config` `recipe_recency: false`.                                                                                                       |
