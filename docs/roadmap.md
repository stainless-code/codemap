...
- [x] **Index staleness surfacing** — `index_freshness.pending_sync` on `context`, MCP tool metadata, and HTTP headers when the watcher debounce queue or in-flight reindex is active. Plan: [`plans/index-freshness-trust-bundle.md`](./plans/index-freshness-trust-bundle.md). Shipped [#149](https://github.com/stainless-code/codemap/pull/149).
...
- [x] **HEAD / index freshness warning** — `index_freshness.commit_drift` + `warning` on `context` / tool metadata; boot stderr on `codemap mcp` / `serve` when concerns remain after prime. Plan: [`plans/index-freshness-trust-bundle.md`](./plans/index-freshness-trust-bundle.md). Shipped [#149](https://github.com/stainless-code/codemap/pull/149).
