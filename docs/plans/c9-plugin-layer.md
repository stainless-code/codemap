...
- **Q2 — Plugin discovery mechanism.** npm peerDep registration (mirrors community-adapter pattern from [`roadmap.md § Strategy`](../roadmap.md#strategy))? Path-glob auto-discovery from `<projectRoot>/.codemap/plugins/`? Config-listed (`.codemap/config.{ts,js,json}` `plugins: [...]`)? Some combination?
