...
**Query batch / file / schema wiring:** **`src/cli/cmd-query-batch.ts`** (argv — `query batch [--stdin | --file]`; JSON body validated with `queryBatchArgsSchema`) + **`handleQueryBatch`** in **`tool-handlers.ts`**. **`src/cli/cmd-resource.ts`** (argv — `file <path>` / `schema`) + **`buildFileRollup` / `buildSchemaCatalog`** exported from **`resource-handlers.ts`** (same payloads as `codemap://files/{path}` / `codemap://schema`).
