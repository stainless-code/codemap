...
function registerExploreTool(server: McpServer, opts: ServerOpts): void {
  server.registerTool(
    ...
    {
      description:
        "Multi-symbol neighborhood survey with budget-capped snippets. Composes `symbol-neighborhood` (once per deduped name) + disk reads. Args: names (non-empty array), depth?, kind?, budget_chars (adaptive default when omitted; snippet source only). Returns {names, rows, snippets, truncated, truncation?} — `truncation.rows` when adaptive row cap hit, `truncation.snippets` when budget hit. Fall back to `query_recipe` symbol-neighborhood.",
...
function registerNodeTool(server: McpServer, opts: ServerOpts): void {
  server.registerTool(
    ...
    {
      description:
        "One-hop symbol survey: `show` center + scoped depth-1 `symbol-neighborhood` + optional inline snippets. When center is unique (`in` or single match), neighborhood filters to that instance's connected files. Args: name, kind?, in?, include_snippets (default false), budget_chars? (adaptive default when snippets enabled and omitted; snippet source only). Returns {center, neighborhood, snippets, truncated, truncation?}. `truncated` only when `include_snippets: true` and snippet budget hit.",
