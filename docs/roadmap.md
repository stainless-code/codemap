...
- **No daemon by default** — `watch` / `mcp --watch` / `serve --watch` are opt-in for inherently long-running modes (sub-100ms cold-start floor preserved for one-shot CLI). `CODEMAP_WATCH=1` is the env-shortcut for IDE / CI launches that can't easily edit the spawn command.
...
- [ ] **Default `--watch` ON for `mcp` / `serve`** — flip `let watch = envWatch;` → default ON; add `--no-watch` flag + `CODEMAP_WATCH=0` env opt-out. One-shot CLI defaults preserved (`query` / `show` / `snippet` keep no watcher). Eliminates "is the index stale?" friction (the most-frequent agent UX issue). Downstream: AST in-memory caching between requests (drops incremental reindex ms → µs; data path already exists). Effort: XS (flag default + test + agent-rule lockstep update + patch changeset).
...
- [ ] **`cmd-mcp.ts --help` resource list parity** — `--help` text currently lists 4 MCP resources (`recipes` / `recipes/{id}` / `schema` / `skill`); 6 ship (add `codemap://files/{path}` and `codemap://symbols/{name}`). Mirror `templates/agents/rules/codemap.md` enumeration so `mcp --help` matches code reality. Effort: XS.
