...
**GitHub Actions:** On **every push to `main`**, [release.yml](../.github/workflows/release.yml) runs. If there are **pending** `.changeset/*.md` files, the action only needs **`GITHUB_TOKEN`** to open/update the Version PR. If there are **no** pending changesets (e.g. a docs-only merge), the action still runs **`npm run release`** (`changeset publish`) so a **just-merged** Version PR can publish — when there is **nothing** to publish, that command should **exit 0**. **`NPM_TOKEN`** (npm [automation token](https://docs.npmjs.com/creating-and-viewing-access-tokens)) must be set as a repo secret for **real** publishes; without it, `changeset publish` can fail in CI even when you are not shipping a new version. The workflow uses **Node** for the publish step (not Bun) so behavior matches `npx changeset publish` locally.
