...
**Generic defaults:** This rule is **project-agnostic**. The bundled rule + skill are served live by the installed `codemap` binary — `codemap agents init` writes thin pointer files to `.agents/` that fetch today's content via `codemap rule` / `codemap skill` (and the matching MCP / HTTP resources). To add **app-specific** triggers / SQL, drop your own files alongside the pointers (e.g. `.agents/rules/my-team.md`, `.agents/skills/my-domain/SKILL.md`) — the bundled pointers stay codemap-managed and auto-refresh on package upgrades.
...
| MCP server (for agent hosts)      | `codemap mcp [--no-watch] [--debounce <ms>]` — JSON-RPC on stdio; one tool per CLI verb. Watcher default-ON since 2026-05. See **MCP** section below.                                                                                                                                                                                                      |
...
| Apply (substrate fix executor)    | `codemap apply <recipe-id> [--params k=v[,k=v]] [--dry-run] [--yes] [--json]` — executes the diff hunks a recipe describes (one per `{file_path, line_start, before_pattern, after_pattern}` row). Q6 gate: TTY prompt; non-TTY needs `--yes` (or `--dry-run`). Q2 (c) all-or-nothing — any conflict aborts before any file is touched.                    |
...
**MCP server (`codemap mcp`)**: stdio MCP (Model Context Protocol) server — agents call codemap as JSON-RPC tools instead of shelling out to the CLI on every read. v1 ships one tool per CLI verb plus seven resources (`codemap://recipes` + `codemap://recipes/{id}` live-read every call so inline `last_run_at` / `run_count` recency stays fresh; `codemap://schema` + `codemap://skill` + `codemap://rule` lazy-cache; `codemap://files/{path}` + `codemap://symbols/{name}` always live):
...
- **Resources:** `codemap://recipes` (catalog — live), `codemap://recipes/{id}` (one recipe — live), `codemap://schema` (live DDL from `sqlite_schema`; lazy-cached), `codemap://skill` (live-assembled `templates/agent-content/skill/*.md`; lazy-cached), `codemap://rule` (live-assembled `templates/agent-content/rule/*.md`; lazy-cached), `codemap://files/{path}` (per-file roll-up: symbols, imports, exports, coverage — live), `codemap://symbols/{name}` (symbol lookup with `{matches, disambiguation?}` envelope; `?in=<path-prefix>` filter mirrors `show --in` — live). Recipe catalogs read live every call so inline `last_run_at` / `run_count` recency reflects mutations during the server lifetime; `schema` / `skill` / `rule` cache because their inputs don't change mid-session.
