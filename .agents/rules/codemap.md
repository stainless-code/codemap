...
| Coverage ingest                | —                  | `bun src/index.ts ingest-coverage <path> [--runtime] [--json]` — Istanbul (`coverage-final.json`), LCOV (`lcov.info`), or V8 (`NODE_V8_COVERAGE=...` directory with `--runtime`); format auto-detected. Joinable to `symbols` for "untested AND dead" queries. Local-only — no SaaS aggregation.           |
