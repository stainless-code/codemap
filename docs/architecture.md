...
| ---------- | ------------------------------------------------ | ----------------------------------------------------- | ------------------------------------------------------------------- |
| **Recipe** | `codemap apply <recipe-id> [--params k=v[,k=v]]` | `apply` — `{recipe, params?, dry_run?, yes?, force?}` | `auto_fixable` + `apply.autoApplyRecipes` + `--force` (recipe-only) |
| **Rows**   | `codemap apply --rows -` or `--rows <file.json>` | `apply_rows` — `{rows, dry_run?, yes?}`               | None — caller owns row trust                                        |
| **Diff**   | `codemap apply --diff-input <file>`              | — (CLI only)                                          | None — `parseUnifiedDiffToRows` → rows                              |
...
**Bundled diff-shape recipes** (emit the row contract; inspect with `codemap query --recipe <id> --format diff-json`): `rename-preview`, `migrate-import-source`, `replace-marker-kind`, `migrate-deprecated`, `deprecated-usages`, `stale-imports` (`auto_fixable: true` on rename/migrate-import/replace-marker only); `add-jsdoc-deprecated` (`auto_fixable: false` — writes need `--force` unless allowlisted). Pair read `deprecated-symbols` with `migrate-deprecated` + `deprecated-usages`. Remainder in [`plans/substrate-apply-utilization.md`](./plans/substrate-apply-utilization.md).
