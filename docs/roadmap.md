...
- [ ] **Rich `context` composer** — expand session bootstrap beyond counts + recipe catalog: ranked hub files, sample markers, optional inlined signatures, fan-in leaders, and links to high-value recipes for the detected intent. Goal: one bootstrap call replaces a common show → explore chain on session start. **Slice 1 shipped in-tree:** `start_here` on non-compact `context` (recipe cards + hub leaders with signatures). Plan: [`plans/rich-context-composer.md`](./plans/rich-context-composer.md). Effort: M.
