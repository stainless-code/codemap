...
- [ ] **`codemap audit --base <ref>`** (v1.x) — worktree+reindex snapshot strategy. v1 shipped `--baseline <prefix>` / `--<delta>-baseline <name>` (B.6 reuse) — see [`architecture.md` § Audit wiring](./architecture.md#cli-usage). v1.x adds `--base <ref>` for "audit against an arbitrary ref I haven't pre-baselined." Plan: [`plans/audit-base.md`](./plans/audit-base.md).
- [ ] **`.codemap/` directory consolidation** — single root + self-managed `.codemap/.gitignore` (blacklist of generated artifacts, mirrors flowbite-react pattern). Migrate `.codemap.db` → `.codemap/index.db` with a one-cycle shim; collapse the per-feature `agents-init.ts` `.gitignore` patching to a one-time `.codemap/` defensive entry. Plan: [`plans/codemap-dir-consolidation.md`](./plans/codemap-dir-consolidation.md).
