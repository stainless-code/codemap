...
- [ ] **Agent transports — MCP server (v1) + HTTP API (v1.x)** — wrap codemap's structural-query surface (`query` / `query_recipe` / `audit` / `context` / `validate` / baseline ops) for agent-host integration. Plan: [`plans/agent-transports.md`](./plans/agent-transports.md). v1 ships `codemap mcp` (stdio + JSON-RPC, one tool per CLI verb, output shape verbatim from `--json`); v1.x adds `codemap serve [--port] [--host 127.0.0.1]` (loopback default; same tool taxonomy / output shape). Builds on every CLI primitive to date — supersedes the prior bullet-form MCP / HTTP entries.
