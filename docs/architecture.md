...
| **`application/`**                           | Pure transport-agnostic engines: `run-index.ts` / `index-engine.ts` (orchestration + indexing); `query-engine.ts` (`executeQuery` / `executeQueryBatch`); `audit-engine.ts` (`runAudit` + `resolveAuditBaselines`); `context-engine.ts` (`buildContextEnvelope`); `validate-engine.ts` (`computeValidateRows` + `toProjectRelative`); `show-engine.ts` (lookup + envelope builders); `query-recipes.ts` + `recipes-loader.ts` (recipe registry); `mcp-server.ts` (MCP tool/resource layer). Engines depend on `db.ts` / `runtime.ts`; **never** on `cli/`. |
