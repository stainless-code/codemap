...
- **Evidence columns** — high-judgment recipes (`boundary-violations`, `deprecated-symbols`, `unimported-exports`, …) may add **`reason`** and **`evidence_json`** on each row — factual detection path (consumer hop, re-export chain, or unresolved-import blind spot on `unimported-exports`); parse before `apply` or deletion.
...
- **Confidence columns** — `coverage-confirmed-dead` adds **`confidence`** (`high` \| `medium`), **`reason`**, **`caller_count`** on static dead exports — `high` only after `ingest-coverage` shows 0% on the symbol.
