...
| **Tier 4 — types on symbols**           | `return_type` / `is_async` on `prefetch`; `@param` / `@returns` / `@throws` → `jsdoc_tags` on `createClient`; **`type-ancestors` / `type-descendants`** on `src/types/hierarchy.ts` (`extends` + `implements` + multi-base)                             |
