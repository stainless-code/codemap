...
| provenance             | TEXT       | `ast` (parse), `heuristic` (callback-synthesis), or NULL (legacy ast-era rows). Moat-A surfaces (`call-path`, `find-call-sites`, `symbol-neighborhood`, impact/trace walks, `call-resolution-stats`) filter `NULL`/`ast` only; opt-in `calls-including-heuristic` |
