...
| **Tier 4 — types on symbols**           | `return_type` / `is_async` on `prefetch`; `@param` / `@returns` / `@throws` → `jsdoc_tags` on `createClient`                                                                                                                                            |
| **Tier 5 — behavioral**                 | `await` in loop (`prefetch`); swallowed catch (`cache.ts`); `@sealed` decorator (`decorated.ts`); `@throws` jsdoc                                                                                                                                       |
...
| **`file_metrics` / complexity**         | `lib/complexity-fixture.ts` — `large-functions`, `deeply-nested-functions`, `high-complexity-untested`                                                                                                                                                  |
