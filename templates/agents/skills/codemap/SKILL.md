...
- **`codemap query --json`** prints a **JSON array** of row objects to stdout on success.
...
**JSON array samples (JSON1):** use **`codemap query --recipe fan-out-sample-json`** — or replace **`GROUP_CONCAT`** with **`json_group_array(to_path)`** in the inner subquery if your SQLite build has JSON1.
