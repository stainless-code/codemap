...
- **Verdict-shaped lints** (severity levels, suppression comments, autofix suggestions) — `knip` / `jscpd` / `eslint --fix`. **Reach for codemap** when you want predicate-as-API analysis you compose yourself — `query --recipe untested-and-dead` / `worst-covered-exports` / `visibility-tags` / `barrel-files` / `deprecated-symbols`. SQL is the API; verdicts are an output mode of recipes (e.g. `--format sarif` / `annotations`), never a primitive. **`codemap apply`** executes explicit diff rows you provide — it does not invent fixes like ESLint autofix does.
