...
- [x] **Apply-engine direction** — diff-shape recipes, `actions[].command`, `auto_fixable`/`--force`, `rename-preview` call/re-export CTEs, `apply --rows` / `apply_rows`, `--diff-input`, `--commit`, `--until-empty`, `apply.autoApplyRecipes`. Follow-on (docs, recipes, tests): [`plans/substrate-apply-utilization.md`](./plans/substrate-apply-utilization.md). Executor checklist: [`plans/apply-engine-direction.md`](./plans/apply-engine-direction.md).
