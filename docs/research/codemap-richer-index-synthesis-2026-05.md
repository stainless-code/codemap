...
| ----------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ------ | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `calls.{line_start, column_start}`                                      | A1, A2 (call-site spans), A3, A4 (column anchoring), A5 (with column_end), A6 | S      | **Synthesis Step 5**           | Reuse existing `buildLineMap` / `offsetToLine` from `parser.ts`; one-shot reindex on schema bump                                                                         |
| `calls.column_end` (vs only column_start)                               | A5 explicit                                                                   | XS     | **Plan PR question**           | Symmetric to start; cheap; needed for column-precise span replace per A4                                                                                                 |
| `exports.{line_start, column_start}` (or `export_locations` table)      | A1, A2 (re-export traceability), A3 (`export_locations`), A4, A5, A6          | S      | **Synthesis Step 7**           | Closes the second rename-preview substrate gap                                                                                                                           |
| Column anchoring on `symbols` / `imports` / `markers` (existing tables) | A4 explicit                                                                   | M      | **Deferred (incremental)**     | Eliminates same-line ambiguity caveat (`const foo = foo();` first-occurrence trap); recipes opt in by emitting columns, fall back to substring match when columns absent |
...
| -------------------------------------------------------------------- | ------------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `import_specifiers(file_path, line, col_start, col_end, name, kind)` | A1, A3 (subset of `references`) | M      | **Deferred** — split `imports.specifiers` JSON blob into typed child rows; unlocks specifier-precise edits, type-only specifier rewrite, dedupe-imports |
| `jsx_usages(file_path, component_name, line_start, col_start)`       | A1, A3 (subset of `references`) | M      | **Deferred** — component renames + prop migration; React-specific app-wide rename                                                                       |
| `references` (generalised) + `scopes`                                | A3, A4                          | XL     | **Deferred** — defer until ≥3 narrower position tables prove demand AND a recipe wants to UNION across them                                             |
| `bindings` (resolved identity per reference)                         | A3 explicit                     | XL     | **Deferred** — subsumed under generalised `references` plan                                                                                             |
| `ranges` (reusable source spans)                                     | A3 explicit                     | M      | **Deferred** — subsumed under generalised `references` plan                                                                                             |
| `symbol_namespace` (value/type/member/default)                       | A3 explicit                     | M      | **Deferred** — needed for binding-grade rename to avoid TS false matches; subsumed under generalised `references` plan                                  |
...
| --- | ------------------------------------------------------------- | ---- | --------- | -------------- | ------------------------------------------------------------------------------------------- |
| 1   | Column anchoring on `symbols` / `exports` / `imports` etc.    | M    | High      | No             | **Synthesis Step 5, 7** (calls + exports first; `symbols` / `imports` / `markers` deferred) |
| 2   | `apply --diff-input <file>` (arbitrary diff source)           | S    | High      | No             | **Synthesis Step 8, 9**                                                                     |
| 3   | `references` table + scope graph                              | XL   | Very high | No             | **Deferred** — incremental position tables first                                            |
...
### Step 5 — `calls.{line_start, column_start}` substrate column (S)
...
### Step 7 — `exports.{line_start, column_start}` + extend `rename-preview.sql` with `re_export_rows` CTE (S)
