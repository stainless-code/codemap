...
TS shape for the JSON emitted by `codemap context`. Stable contract; agents can key off field names. Non-compact payloads include **`start_here`**: inline **`index_summary`**, intent-ranked recipe cards (`tool: "query_recipe"`), and **`hub_leaders`** (top fan-in files with exported-symbol signatures; optional **`snippet`** when MCP/HTTP `include_snippets: true`).
