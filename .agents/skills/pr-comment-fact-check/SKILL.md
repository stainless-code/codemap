...
Adapted from `PaySpace/merchant-dashboard-v2` `.agents/skills/pr-comment-fact-check/SKILL.md` (2026-04). The process (pull → group → fact-check → categorize → apply/reply/resolve) and the GraphQL/REST commands are universal and kept verbatim. Hallucination patterns rewritten for codemap (the React 19 / TanStack Form / TanStack Store / React Query patterns from the source were dropped — codemap doesn't use any of those — and replaced with codemap-thesis-shaped ones grounded in `docs/roadmap.md § Non-goals`). The "Resolve-on-merge-gate exception" trimmed to a single short subsection since codemap's `main` doesn't currently require conversation resolution.
